...
import { siteGateGuard } from './core/site-gate/site-gate.guard';

const routes: Routes = [
  {
    path: '',
    // Erlaubt ohne Login nur Routen mit `data: { oeffentlich: true }` --
    // siehe site-gate.guard.ts.
    canActivateChild: [siteGateGuard],
    children: [
      ...
      },
      {
        ...
        loadChildren: () => import('./modules/oeffentlich/oeffentlich.module')
          .then(m => m.OeffentlichModule)
      },
      {
      ...
      },
      {
        ...
        // Offiziell freigegeben: ohne Login zugaenglich, siehe OEFFENTLICH_FREIGEGEBENE_IDS
        // in modules/oeffentlich/pages/oeffentlich.page.ts.
        data: { oeffentlich: true },
