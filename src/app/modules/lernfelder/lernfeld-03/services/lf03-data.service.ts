...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf03.models';

const STORAGE_KEY = 'lf03-progress';

@Injectable({ providedIn: 'root' })
export class Lf03DataService {
  private readonly base = 'assets/lernfelder/lernfeld-03/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf03-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf03-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf03 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
