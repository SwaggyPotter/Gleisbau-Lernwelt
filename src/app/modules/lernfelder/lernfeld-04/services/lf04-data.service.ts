...
const STORAGE_KEY = 'lf04-progress';

@Injectable({ providedIn: 'root' })
export class Lf04DataService {
  private readonly base = 'assets/lernfelder/lernfeld-04/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf04-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf04-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf04 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
