...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf05.models';

const STORAGE_KEY = 'lf05-progress';

@Injectable({ providedIn: 'root' })
export class Lf05DataService {
  private readonly base = 'assets/lernfelder/lernfeld-05/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf05-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf05-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf05 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
