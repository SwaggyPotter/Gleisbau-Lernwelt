...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf06.models';

const STORAGE_KEY = 'lf06-progress';

@Injectable({ providedIn: 'root' })
export class Lf06DataService {
  private readonly base = 'assets/lernfelder/lernfeld-06/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf06-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf06-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf06 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
