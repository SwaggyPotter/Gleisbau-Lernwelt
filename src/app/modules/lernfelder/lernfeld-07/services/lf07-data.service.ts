...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf07.models';

const STORAGE_KEY = 'lf07-progress';

@Injectable({ providedIn: 'root' })
export class Lf07DataService {
  private readonly base = 'assets/lernfelder/lernfeld-07/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf07-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf07-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf07 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
