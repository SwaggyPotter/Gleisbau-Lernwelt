...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf08.models';

const STORAGE_KEY = 'lf08-progress';

@Injectable({ providedIn: 'root' })
export class Lf08DataService {
  private readonly base = 'assets/lernfelder/lernfeld-08/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf08-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf08-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf08 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
