...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf09.models';

const STORAGE_KEY = 'lf09-progress';

@Injectable({ providedIn: 'root' })
export class Lf09DataService {
  private readonly base = 'assets/lernfelder/lernfeld-09/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf09-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf09-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf09 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
