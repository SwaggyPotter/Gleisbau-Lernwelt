...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf10.models';

const STORAGE_KEY = 'lf10-progress';

@Injectable({ providedIn: 'root' })
export class Lf10DataService {
  private readonly base = 'assets/lernfelder/lernfeld-10/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf10-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf10-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf10 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
