...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf11.models';

const STORAGE_KEY = 'lf11-progress';

@Injectable({ providedIn: 'root' })
export class Lf11DataService {
  private readonly base = 'assets/lernfelder/lernfeld-11/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf11-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf11-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf11 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
