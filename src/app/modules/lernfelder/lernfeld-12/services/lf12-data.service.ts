...
const STORAGE_KEY = 'lf12-progress';

@Injectable({ providedIn: 'root' })
export class Lf12DataService {
  private readonly base = 'assets/lernfelder/lernfeld-12/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf12-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf12-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf12 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
