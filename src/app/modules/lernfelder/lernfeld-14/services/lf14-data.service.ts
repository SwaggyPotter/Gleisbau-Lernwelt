...
import { BlockProgress, ContentBlock, QuizFile } from '../models/lf14.models';

const STORAGE_KEY = 'lf14-progress';

@Injectable({ providedIn: 'root' })
export class Lf14DataService {
  private readonly base = 'assets/lernfelder/lernfeld-14/';
  ...
  getContent(): Observable<ContentBlock[]> {
    if (!this.content$) {
      this.content$ = this.http.get<{ blocks: ContentBlock[] }>(`${this.base}lf14-content.json`).pipe(
        map(res => res.blocks ?? []),
        shareReplay(1),
      );
    }
  ...
  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}lf14-quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }
  ...
  loadProgress(): BlockProgress {
    ...
    try {
      ...
      console.warn('lf14 progress parse failed', e);
      return { completedBlocks: [], quizStats: {} };
    }
  }
