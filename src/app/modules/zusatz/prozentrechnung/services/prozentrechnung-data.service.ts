import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { shareReplay } from 'rxjs/operators';
import { Observable } from 'rxjs';
import { BlockProgress, QuizFile } from '../models/prozentrechnung.models';
import { ProfilSyncService } from '../../../../core/auth/services/profil-sync.service';

const STORAGE_KEY = 'prozentrechnung-progress';

@Injectable({ providedIn: 'root' })
export class ProzentrechnungDataService {
  private readonly base = 'assets/zusatz/prozentrechnung/';

  private quiz$?: Observable<QuizFile>;

  constructor(
    private readonly http: HttpClient,
    private readonly profilSync: ProfilSyncService,
  ) {}

  getQuiz(): Observable<QuizFile> {
    if (!this.quiz$) {
      this.quiz$ = this.http.get<QuizFile>(`${this.base}quiz.json`).pipe(shareReplay(1));
    }
    return this.quiz$;
  }

  loadProgress(): BlockProgress {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) {
      return { quizStats: {} };
    }
    try {
      return JSON.parse(raw) as BlockProgress;
    } catch (e) {
      console.warn('prozentrechnung progress parse failed', e);
      return { quizStats: {} };
    }
  }

  saveProgress(next: Partial<BlockProgress>): BlockProgress {
    const merged: BlockProgress = { ...this.loadProgress(), ...next };
    localStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
    return merged;
  }

  recordQuizResult(questionId: string, correct: boolean): BlockProgress {
    const state = this.loadProgress();
    const prev = state.quizStats[questionId] ?? { correct: 0, wrong: 0 };
    const quizStats = {
      ...state.quizStats,
      [questionId]: {
        correct: prev.correct + (correct ? 1 : 0),
        wrong: prev.wrong + (correct ? 0 : 1),
      },
    };
    this.profilSync.melde('zusatz:prozentrechnung', correct);
    return this.saveProgress({ quizStats });
  }
}
